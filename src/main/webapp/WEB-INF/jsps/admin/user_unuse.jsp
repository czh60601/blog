...
<%@ include file="../banner.jsp"%>
    ...
    <div class="row">
    <div class="col-lg-12">
	  	<form id="list" action="${rootUrl}/background/user/sys/unuse" method="post">
	    	...
	    	<thead>
	    		<tr>
	    			...
	    			<th colspan="3">
	    				...
	    				<c:if test="${logUser.hasItem('background.sys_user','del')}">
	    				<input type="button" class="btn btn-danger btn-sm" onclick="deletes()" value="彻底删除"/>
	    				</c:if>
	    			</th>
	    		</tr>
	    		<tr>
	...
	<div class="modal-dialog">
		<div class="modal-content card">
			...
			<div class="modal-footer">
				...
				<button type="button" onclick="saveUpdate()" class="btn btn-primary">恢复</button>
			</div>
		</div><!-- /.modal-content -->
	</div><!-- /.modal -->
	</div>
    ...
    <script type="text/javascript" src="/wat/js/tongji2.js"></script>
    <script type="text/javascript">
		...
		function deletes(){
			...
			if(ids.length){
				var ret = confirm("删除后将不可恢复,确定彻底删除这"+ids.length+"个用户?");
				if(ret){
    	...
    	function getInfo(id){
    		try{
	    		$.post("${rootUrl}/background/user/edit?status=0&uid="+id+"&queryType="+ResultEnum.TYPE_INFO,function(result){
